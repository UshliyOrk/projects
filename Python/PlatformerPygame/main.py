...
class Player(pygame.sprite.Sprite):
	...
	def __init__(self):
		...
		self.walkLeft = [iml(f'l{i}.png') for i in range(1, 7)]
		self.walkRight = [iml(f'r{i}.png') for i in range(1, 7)]
		self.stand = iml('idle.png')
		self.frame()
	...
	def frame(self):
		...
		try:
			if self.left:
				self.image = self.walkLeft[self.anim//3]
				self.anim += 1
			elif self.right:
				self.image = self.walkRight[self.anim//3]
				self.anim += 1
		except:
			self.left = False
			self.right = False
